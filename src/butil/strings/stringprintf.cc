...
// 
// 关闭警告：gcc7 当在 StringAppendVT 函数中 vsnprintfT 的第一个参数是 NULL 时
// 将警告。
...
// 
// 重载 vsnprintf 和 vswprintf 的包装器。 buf_size 参数是缓冲区的大小。这些返回格
// 式化字符串中不包含 NUL 终止符的字符数。如果缓冲区不足以容纳格式化的字符串而没有截断，
// 它们将返回完全格式化字符串（ Windows 上的 vsnprintf 和 vswprintf ）或 -1（ 
// POSIX 平台上的 vswprintf ）中的字符数量。
