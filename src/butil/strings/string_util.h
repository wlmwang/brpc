...
//
//  不可移植的 wprintf 说明符：
//  - 's' 和 'c' 没有 'l' 长度修饰符。因为除了 Windows 可以让 %s 和 %c 修饰符对 char 
//    数据进行等同 wchar_t 一样操作，其他系统都使用 %ls 和 %lc 作为 wchar_t 数据
//  - 'S' 和 'C' ，它们在 Windows 以外的所有系统上对 wchar_t 数据进行操作，将它们视为 
//    char 数据。其他系统使用 %ls 和 %lc 处理 wchar_t 数据
//  - 'F' ，这是没有确定的 Windows wprintf 文档
//  - 'D', 'O', 和 'U' ，它们已被弃用，并且在所有系统上都不可用。 改用 %ld ，%lo 和 
//    %lu
//
...
// 线程安全函数返回全局唯一空字符串的引用。
// 
// 构建一个新的空字符串对象（只需要几条指令将长度设置为 0）比获取由这些函数返回的空字
// 符串单例（这需要线程安全的单例访问）更快。
// 
// 因此，请勿将其作为默认构建空字符的一般用途替代品。只有一种情况应该使用这些函数：需
// 要通过引用返回字符串的函数（例如，作为类成员访问器），并且没有要使用的空字符串（例
// 如，在错误情况下）。这些不应该用作初始值设定项，函数参数或函数的返回值，这些值通过
// 值或超参数返回。
