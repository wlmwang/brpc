...
// @tips
// 1. 内存屏障，其实本质上是为了解决因为编译器优化和 CPU 对 CPU Cache/寄存器(Register) 
//    的使用，从而导致对内存的操作不能够及时反映出来的一类问题。比如 CPU 写入后，读出来的
//    值可能是旧的内容。
// 2. 内存屏障，是一类同步屏障指令，是 CPU 或编译器在对内存随机访问操作中的一个同步点，使
//    得此点之前的所有读写操作都执行后才可以开始执行此点之后的操作。大多数现代计算机为了提
//    高性能而采取乱序执行，这使得内存屏障成为必须。
// 
// CPU 一般情况下是不直接读写内存的 (emmintrin.h 应用例外)，其一般流程为：
// 读取内存数据到 CPU Cache -->  CPU 读取 CPU Cache/寄存器(Register) -->  CPU 的计
// 算(CPU ALU 单元) --> 将结果写入 CPU Cache /寄存器(Register) --> 写回数据到内存
// 
// 语义上，内存屏障之前的所有写操作都要写入内存；内存屏障之后的读操作都可以获得同步内存屏障
// 之前的写操作的结果。因此，对于敏感的程序块，写操作之后、读操作之前可以插入内存屏障。
// 
// 内存屏障提供了两种属性：
// 1. 它们保留了外部可见的程序顺序。通过确保所有的、屏障两侧的指令表现出正确的程序顺序，比
//    如从其他 CPU 观察。
